...
| `root` Application `SyncFailed`: "could not find traefik.io/IngressRoute ... Make sure the CRD is installed" | The `IngressRoute`/`Middleware` objects in `argocd-apps/ingress-routes.yaml` were being applied in the same sync wave as the `traefik` child Application, before Traefik's chart (and its bundled CRDs) had synced. Fixed by giving those objects `sync-wave: "1"` (later than traefik's `"0"`) |
